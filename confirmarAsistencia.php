...
if ($estado_curso == 'G'){ //Asistencia previamente guardada.
	...
	$sql = "UPDATE asistencias_cursos
			SET 	Estado_curso = 'C', Fecha = '$fecha_actual'
			WHERE 	Dictadosid_curso = $id_curso
					AND Docentesid_docente = $id_docente";
	
	//Actualizo en la DB.
	if (mysql_query($sql,$con)) {
		echo "Registro actualizado exitosamente(asistencias_cursos)";
	}else{
		echo "Error al actualizar registro: " . mysql_error($con);
	}
}else{ //Asistencia nunca guardada y confirmada.
	...
	VALUES ($id_curso,$id_docente,'C','$fecha_actual')";
